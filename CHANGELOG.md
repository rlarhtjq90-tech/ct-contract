...
- **주요 기능:**
  ...
  - 도급계약(`/projects`) 목록: "프로젝트명" 라벨 → "계약명"으로 통일, 상태 컬럼 제거
  - 하도급계약(`/subcontracts`) 목록: 상태 컬럼 제거 (STATUS_LABELS/COLORS 미사용 코드 정리)
...
### 2026-07-06
- 도급계약(`/projects`) 목록: "프로젝트명" 표기를 "계약명"으로 변경 (검색 placeholder, 테이블 헤더, 등록 폼 라벨)
- 도급계약·하도급계약 목록 대시보드에서 상태 컬럼(뱃지) 제거, 관련 미사용 STATUS_LABELS/STATUS_COLORS 상수 삭제
