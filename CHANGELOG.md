...
- **상태:** 개발 진행 중 (Phase 3) — 네비게이션 재구조화 + 도급 기성현황 신규 구현, DB 초기화 후 테스트 대기
- **주요 기능:**
  - 로그인 (JWT, admin/pm/viewer — 공통 비밀번호 `ct1234!`)
  - 사이드바 아코디언: 도급계약(발주처·기성현황) / 하도급계약(하도급사·기성현황)
  - 발주처 관리 `/clients` (도급계약 > 발주처)
  - 하도급사 관리 `/subcontractors` (하도급계약 > 하도급사) — 신규
  - 도급계약 관리 `/projects` + 변경계약·삭제요청
  - 하도급계약 관리 `/subcontracts` + 변경계약·삭제요청
  - 도급 기성현황 `/project-billings` (발주처 기성 월별 입력·승인) — 신규
  - 하도급 기성현황 `/billings` (하도급사 기성 월별 입력·승인)
  - 대시보드 KPI + 차트 (Recharts)
  - RBAC: admin=전체, pm=등록·변경·삭제요청, viewer=조회만
- **알려진 이슈:** DB 초기화 완료 — 테스트 데이터 미입력 상태 (TC-01부터 진행 필요)
